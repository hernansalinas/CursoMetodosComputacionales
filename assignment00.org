...
* Pandas: datafraes and series.
We have the following. Every row of tis array coresponds to a aSeries, whcih is a special type of dictionary. Note that the even and odd tags (or rather, keys) automatically turn into attributes.
#+begin_src python :results output :exports both
import pandas as pd

numbers = {'even': {0: 0, 1: 2, 2: 4, 3: 6}, 'odd': {0: 0, 1: 2, 2: 4, 3: 5}}

data_frame = pd.DataFrame(numbers)
print(data_frame.shape)
print(type(data_frame['even']))
print(data_frame.even[3])
#+end_src
...
: 6
We can filter dataframes with the characters [[]], for indtance, by saying data_frame[[[['odd']]]] to filter columns and data_frame.loc[[[[2]]]] to filter rows.

#+begin_src python :results output :exports both
def graph(data, num_bins) :
    data_size = data.size
    ...
    l = plt.plot(bins, y*data_size*abs(bins[0]-bins[1]), 'r--', linewidth=1)
...
#+begin_src python
# we begin by getting the array of data supposedly obtained from a random experiment
def random_experiment_info(data, data_size) :
    np.random.seed(0)
    data_set = np.random.choice(data, data_size)

    mu = np.mean(data_set)
    st_dev = np.std(data_set)
    st_error = st_dev / np.sqrt(data_size)

    return mu, st_dev, st_error
#+end_src
